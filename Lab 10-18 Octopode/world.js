function World() {
  ...
  this.octopi = [];
  this.loadOctopi(4);
  //variables to move the canvas to align with a specific snake
  this.movin;
  this.mNum;
...
World.prototype.loadMovers = function (n) {
  for(let i = 0; i< n; i ++){
    this.mover[i] = new Mover(Math.random()*this.cnvMain.width - this.cnvMain.width/2, Math.random()*this.cnvMain.height - this.cnvMain.height/2, 20)
  }
}
